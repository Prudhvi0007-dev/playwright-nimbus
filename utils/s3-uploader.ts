...
export class S3Uploader {
  ...
  async uploadFile(localFilePath: string, s3Key: string): Promise<UploadedFile> {
    try {
  ...
  async uploadDirectory(localDir: string, s3Prefix: string): Promise<UploadedFile[]> {
  ...
  private getFileType(filePath: string): 'video' | 'screenshot' | 'trace' | 'report' | 'other' {
    const ext = path.extname(filePath).toLowerCase();
    const fileName = path.basename(filePath).toLowerCase();
    
    if (ext === '.webm' || ext === '.mp4') return 'video';
    if (ext === '.png' || ext === '.jpg' || ext === '.jpeg') return 'screenshot';
    if (ext === '.zip' || fileName.includes('trace')) return 'trace';
    if (ext === '.html' && fileName.includes('report')) return 'report';
    return 'other';
  }
