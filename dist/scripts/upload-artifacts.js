    ...
    try {
        ...
        for (const { local, s3 } of artifactDirs) {
            if (fs.existsSync(local)) {
                console.log(`\n📁 Uploading ${local}/...`);
                const raw = await uploader.uploadDirectory(local, s3);
                            ...
                            : name.match(/\.(png|jpe?g)$/i) ? 'screenshot'
                                : name.match(/trace|\.zip$/i) ? 'trace'
                                    : (name === 'index.html' || /report/i.test(name)) ? 'report'
                                        : 'other';
                        return { url, key: `${s3}/${name}`, type, size: 0, name };
                    }
                    if (item && typeof item === 'object' && 'url' in item) {
                        return item;
                    }
                    return { url: String(item), key: '', type: 'other', size: 0, name: String(item) };
                });
...
function generateIndexHtml(files, runId) {
